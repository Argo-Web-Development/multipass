...
static QString pid_file()
{
    if (mu::is_snap())
    {
        ...
        //        return QString("%1/dnsmasq.pid").arg(QString(mu::snap_common_dir()));
    }
    else
    {
        return QString();
...
profile %1 flags=(attach_disconnected) {
  ...
  %4/dnsmasq.leases rw,           # Leases file
  %4/dnsmasq.hosts r,             # Hosts file
  ...
  %5 w,            # pid file
    ...
    return profile_template.arg(apparmor_profile_name(), signal_peer, root_dir, data_dir, pid);
}
